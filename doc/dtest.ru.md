...
Утилита `dtest` выполняет запросы к БД, указанные в спецификациях конфиг-файла, и формирует отчет о результатах их выполнения в файлах формата HTML.
...
См. тестовые конфиг-файлы `test/dtest_test_<database>.py` чтобы составить представление обо всех параметрах и их назначении.
...
| `RUN_REPORT_TEMPLATE`  |                       | Имя файла с пользовательским Jinja2 шаблоном отчета о результатах работы утилиты. См. пример `cfg/dtest_sample_run.html.jinja`. |
| `SPEC_REPORT_TEMPLATE` |                       | Имя файла с пользовательским Jinja2 шаблоном отчета по одной спецификации. См. пример `cfg/dtest_sample_spec.html.jinja`.       |
