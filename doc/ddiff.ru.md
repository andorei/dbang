...
Утилита `ddiff` выполняет запросы к двум БД, сравнивает результаты и формирует HTML-отчет о найденных расхождениях.
...
Вместо извлечения миллионов строк из двух БД можно извлечь намного меньше агрегированных строк, полученных группировкой по выбранным столбцам. Затем сравнить два набора агрегированных строк и, в случае обнаружения расхождений, копнуть глубже и найти строки в обеих БД, отвечающие за найденные расхождения.
...
Здесь строки таблиц группируются по дате первого дня месяца продажи и коду торговой точки (point of sales). Сочетание `(month, pos_code)` является первичным ключом для результирующих наборов строк.
...
Строки с продажами за март 2023 на точке 52 отличаются столбцами `sum_qty` и `sum_amount`. Данные продаж по другим месяцам и точкам продаж не имеют расхождений.

Чтобы найти отличающиеся строки в двух БД нужно выбрать строки только для месяцев и точек продаж, для которых есть расхождения агрегированных строк. Понадобится извлечь около 13000 строк из каждой БД (см. столбец `count_`) и сравнить их.
...
Ниже пример конфиг-файла для утилиты `ddiff` со спецификацией, использующей рассмотренные запросы (с незначительными изменениями):
...
Посмотрим, как это работает, на примере спецификации `"current"` из конфиг-файла `ddiff_test_sqlite.py`:
...
| `RUN_REPORT_TEMPLATE`  |                       | Имя файла кастомного Jinja2-шаблона отчета по конфиг-файлу. См. пример `cfg/ddiff_sample_run.html.jinja`.  |
| `SPEC_REPORT_TEMPLATE` |                       | Имя файла кастомного Jinja2-шаблона отчета по спецификации. См. пример `cfg/ddiff_sample_spec.html.jinja`. |
