...
Здесь строки таблиц группируются по дате первого дня месяца продажи и коду торговой точки (point of sales). Сочетание `(month, pos_code)` является уникальным идентификатором (первичным ключом) для результирующих наборов строк.
...
Как видим, строки с продажами за март 2023 на точке 52 отличаются столбцами `sum_qty` и `sum_amount`. Данные продаж по другим месяцам и точкам продаж не имеют расхождений.
...
Посмотрим, как это работает, на примере спецификации `"current"` из конфиг-файла `ddiff-test-sqlite.py`:
