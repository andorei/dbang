...
Конфиг-файлы утилит **dbang** имеют следующую структуру:
...
В конфиг-файлах для `dfifo` и `hedwig` не используются определения источников данных из `sources`. Поэтому не нужно импортировать `sources` в эти конфиг файлы (если вы не решили держать переменные `MAIL_*` для подключения к почтовому серверу в `sources.py`). 
...
Словарь (dict) `specs` содержит спецификации, которые сами являются словарями (dict). Перечень параметров спецификации см. в документации по конкретной утилите и в тестовых конфиг-файлах.
...
Данный раздел относится к конфиг-файлам всех утилит, кроме `dfifo` и `hedwig`.
