...
"""
    ...
    "csv_test_test": {
        ...
        "file": "test.csv",
        "args": ['one', 'two'],
        "insert_actions": "insert into dput_csv_test_test (code, name, alpha2, alpha3) values (:1, :2, :3, :4)",
        "insert_data": lambda row: (row[3], row[0], row[1], row[2]),
        "process_actions": "delete from ida where iload = :1",
        # test setup and teardown
        "setup": CREATE_TABLE_TEST.format('dput_csv_test_test'),
        "upset": DROP_TABLE.format('dput_csv_test_test')
    },
    "csv_parts_test": {
        "tags": ['csv', 'ida'],
        "file": "test_000???.csv",
    ...
    "json_test_test": {
        "encoding": "UTF-8",
        "file": "test.json",
        "insert_actions": "insert into dput_json_test_test (code, name, alpha2, alpha3) values (:1, :2, :3, :4)",
        "insert_data": lambda row: (row["code"], row["name"], row["alpha2"], row["alpha3"]),
        "process_actions": "delete from ida where iload = :1",
        # test setup and teardown
        "setup": CREATE_TABLE_TEST.format('dput_json_test_test'),
        "upset": DROP_TABLE.format('dput_json_test_test')
    },
    "json_parts_test": {
        "tags": ['ida'],
        "file": "test_000???.json",
    ...
    "xlsx_test_test": {
        "file": "test.xlsx",
        "insert_actions": "insert into dput_xlsx_test_test (code, name, alpha2, alpha3) values (:1, :2, :3, :4)",
        "insert_data": lambda row: (row[3], row[0], row[1], row[2]),
        "process_actions": "delete from ida where iload = :1",
        # test setup and teardown
        "setup": CREATE_TABLE_TEST.format('dput_xlsx_test_test'),
        "upset": DROP_TABLE.format('dput_xlsx_test_test')
    },
    "xlsx_parts_test": {
        "tags": ['ida'],
        "file": "test_000???.xlsx",
    ...
    "csv_test_error": {
        "tags": ['csv'],
        "file": "test.csv",
        "insert_actions": "insert into dput_csv_test_error (code, name, alpha2, alpha3) values (:1, :2, :3, :4)",
        "insert_data": lambda row: (row[3], row[0], row[1], row[2]),
        ...
        "upset": DROP_TABLE.format('dput_csv_test_error')
    },
    "nested_00_ida": {
        "tags": ['ida', 'csv', 'nested'],
    ...
    "nested_01_test": {
        ...
        "encoding": "UTF-8",
        "insert_actions": [
            "insert into nested_01_test_region (region, contains) values (:1, :2)",
            "insert into nested_01_test_countries (region, code, name) values (:1, :2, :3)"
        ],
        "insert_data": [
            lambda row: (row['region'], len(row['countries'])),
        ...
        "process_actions": [
            "delete from ida where iload = :1",
            "delete from nested_01_test_countries where :1 is not null",
            "delete from nested_01_test_region where :1 is not null"
        ],
        # test setup and teardown
        "setup": [
        ...
        "upset": [
            DROP_TABLE.format('nested_01_test_region'),
            DROP_TABLE.format('nested_01_test_countries')
        ]
    },
    "nested_01_keygen": {
        "tags": ['json', 'nested'],
        ...
        "encoding": "UTF-8",
        "insert_actions": [
            "insert into nested_01_keygen_region (region_id, region, contains) values (:1, :2, :3)",
            "insert into nested_01_keygen_countries (region_id, code, name) values (:1, :2, :3)"
        ],
        "insert_data": [
        ...
        "process_actions": [
            "delete from ida where iload = :1",
            "delete from nested_01_keygen_countries where :1 is not null",
            "delete from nested_01_keygen_region where :1 is not null"
        ],
        # test setup and teardown
        "setup": [
            DROP_TABLE.format('nested_01_keygen_region'),
            CREATE_TABLE_REGION.format('nested_01_keygen_region'),
            DROP_TABLE.format('nested_01_keygen_countries'),
