...
specs = {
    ...
    "selected_ida_test": {
        ...
        "validate_actions": [
            ...
            update ida_lines set
                ...
                ierrm = 'Not a vowel.'
            where iload = ?
                and substring(c2, 1, 1) not in ('A', 'E', 'I', 'O', 'U')
            """
        ],
    ...
    "csv_test_test": {
        ...
        "file": "test.csv",
        "args": ['one', 'two'],
        "insert_actions": "insert into dput_csv_test_test (code, name, alpha2, alpha3) values (?, ?, ?, ?)",
        "insert_data": lambda row: (row[3], row[0], row[1], row[2]),
        "process_actions": "delete from ida where iload = ?",
        # test setup and teardown
        "setup": CREATE_TABLE_TEST.format('dput_csv_test_test', 'dput_csv_test_test'),
        "upset": "drop table if exists dput_csv_test_test"
    },
    "csv_parts_test": {
        "tags": ['csv', 'ida'],
        "file": "test_000???.csv",
    ...
    "json_test_test": {
        "encoding": "UTF-8",
        "file": "test.json",
        "insert_actions": "insert into dput_json_test_test (code, name, alpha2, alpha3) values (?, ?, ?, ?)",
        "insert_data": lambda row: (row["code"], row["name"], row["alpha2"], row["alpha3"]),
        "process_actions": "delete from ida where iload = ?",
        # test setup and teardown
        "setup": CREATE_TABLE_TEST.format('dput_json_test_test', 'dput_json_test_test'),
        "upset": "drop table if exists dput_json_test_test"
    },
    "json_parts_test": {
        "tags": ['ida'],
        "file": "test_000???.json",
    ...
    "xlsx_test_test": {
        "file": "test.xlsx",
        "insert_actions": "insert into dput_xlsx_test_test (code, name, alpha2, alpha3) values (?, ?, ?, ?)",
        "insert_data": lambda row: (row[3], row[0], row[1], row[2]),
        "process_actions": "delete from ida where iload = ?",
        # test setup and teardown
        "setup": CREATE_TABLE_TEST.format('dput_xlsx_test_test', 'dput_xlsx_test_test'),
        "upset": "drop table if exists dput_xlsx_test_test"
    },
    "xlsx_parts_test": {
        "tags": ['ida'],
        "file": "test_000???.xlsx",
    ...
    "csv_test_error": {
        "tags": ['csv'],
        "file": "test.csv",
        "insert_actions": "insert into dput_csv_test_error (code, name, alpha2, alpha3) values (?, ?, ?, ?)",
        "insert_data": lambda row: (row[3], row[0], row[1], row[2]),
        "process_actions": "update ida set istat = 2, imess = 'Just testing' where iload = ?",
        # test setup and teardown
        "setup": CREATE_TABLE_TEST.format('dput_csv_test_error', 'dput_csv_test_error'),
        "upset": "drop table if exists dput_csv_test_error"
    },
    "nested_00_ida": {
        "tags": ['ida', 'csv', 'nested'],
    ...
    "nested_01_test": {
        ...
        "encoding": "UTF-8",
        "insert_actions": [
            "insert into nested_01_test_region (region, \"contains\") values (?, ?)",
            "insert into nested_01_test_countries (region, code, name) values (?, ?, ?)"
        ],
        "insert_data": [
            lambda row: (row['region'], len(row['countries'])),
        ...
        "process_actions": [
            "delete from ida where iload = ?",
            "delete from nested_01_test_countries where ? is not null",
            "delete from nested_01_test_region where ? is not null"
        ],
        # test setup and teardown
        "setup": [
            CREATE_TABLE_REGION.format('nested_01_test_region', 'nested_01_test_region'),
            CREATE_TABLE_COUNTRIES.format('nested_01_test_countries', 'nested_01_test_countries')
        ],
        "upset": [
            "drop table if exists nested_01_test_region",
            "drop table if exists nested_01_test_countries"
        ]
    },
    "nested_01_keygen": {
        "tags": ['json', 'nested'],
        ...
        "encoding": "UTF-8",
        "insert_actions": [
            "insert into nested_01_keygen_region (region_id, region, \"contains\") values (?, ?, ?)",
            "insert into nested_01_keygen_countries (region_id, code, name) values (?, ?, ?)"
        ],
        "insert_data": [
        ...
        "process_actions": [
            "delete from ida where iload = ?",
            "delete from nested_01_keygen_countries where ? is not null",
            "delete from nested_01_keygen_region where ? is not null"
        ],
        # test setup and teardown
        "setup": [
            CREATE_TABLE_REGION.format('nested_01_keygen_region', 'nested_01_keygen_region'),
            CREATE_TABLE_COUNTRIES.format('nested_01_keygen_countries', 'nested_01_keygen_countries')
        ],
        "upset": [
            "drop table if exists nested_01_keygen_region",
            "drop table if exists nested_01_keygen_countries"
        ]
    },
    "flatten_ida_test": {
