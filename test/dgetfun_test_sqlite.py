...
def fun_source():
    """
    Returns lists of tuples FUN_COUNT times.
    """
    ...
    if FUN_COUNT:
        FUN_COUNT -= 1
        return [('hello', 'Привет', None, n, 0.0 + n, _date, _ts) for n in range(1, 101)]
    else:
        return []
...
def fun_2_source():
    """
    Returns lists of tuples FUN_2_COUNT times.
    """
    from datetime import date, datetime
    _date = date.today()
    _ts = datetime.now()
...
specs = {
    ...
    "by-100": {
        "file": "dget_%(datetime)s_%(seqn)06i_by-100.csv",
        "rows_per_file": 100,
        "query": """
            with recursive numbers (n) as (
                select 0 as n from dual
    ...
    "by-100.zip": {
        "file": "dget_%(datetime)s_%(seqn)06i_by-100.xlsx.zip",
        "rows_per_file": 100,
        "query": """
            with recursive numbers (n) as (
                select 0 as n from dual
    ...
    "fun_by_70": {
        "tags": ['fun'],
        "source": fun_2_source,
        "file": "dget_%(datetime)s_%(seqn)06i_fun_by_70.json",
        "rows_per_file": 70,
        "setup": fun_2_setup,
        "header": ['Q', 'hello', 'null', 'int', 'float', 'date', 'timestamp']
    },
